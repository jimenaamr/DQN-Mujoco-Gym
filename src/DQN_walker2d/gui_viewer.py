...
class TkLiveViewer:
    ...
    def __init__(
        ...
        self.initial_scale: float = float(initial_scale)
        self.initial_w: int | None = initial_w
        self.initial_h: int | None = initial_h
    ...
    def _set_initial_geometry(self) -> None:
        """Set initial window size relative to screen (or explicit pixels)."""
        if self._state is None:
            return

        screen_w: int = int(self._state.root.winfo_screenwidth())
        screen_h: int = int(self._state.root.winfo_screenheight())

        if self.initial_w is not None and self.initial_h is not None:
            win_w: int = int(max(1, self.initial_w))
            win_h: int = int(max(1, self.initial_h))
        else:
            scale: float = float(self.initial_scale)
            scale = max(self.min_scale, min(self.max_scale, scale))
            win_w = int(max(1, round(float(screen_w) * scale)))
            win_h = int(max(1, round(float(screen_h) * scale)))

        x: int = 0
        y: int = 0
        if self.center:
            x = int(max(0, (screen_w - win_w) // 2))
            y = int(max(0, (screen_h - win_h) // 2))

        self._state.root.geometry(f"{win_w}x{win_h}+{x}+{y}")
        self._state.root.minsize(width=320, height=240)
    ...
    def _frame_to_photo(self, frame_rgb: np.ndarray) -> tk.PhotoImage:
        ...
        Preference:
            1) Pillow (fast, robust).
            2) Fallback to PNG encode (slower) if Pillow is unavailable.

        Args:
            frame_rgb: RGB uint8 frame with shape (H, W, 3).

        Returns:
            Tk-compatible PhotoImage object.
        """
        if frame_rgb.ndim != 3 or frame_rgb.shape[2] != 3:
            raise ValueError(f"Expected frame_rgb as (H,W,3), got {frame_rgb.shape}")
        if frame_rgb.dtype != np.uint8:
        ...
        # Fast/robust path: Pillow
        if Image is not None and ImageTk is not None:
            img: Any = Image.fromarray(frame_rgb, mode="RGB")
            photo: Any = ImageTk.PhotoImage(image=img, master=self._state.root)  # type: ignore[arg-type]
            return photo  # type: ignore[return-value]

        # Fallback: PNG (your original approach)
