...
class _RainbowQNet(nn.Module):
    ...
    Then it splits into dueling distributional heads (C51) using NoisyLinear (optional):
    ...
    def __init__(
        ...
        Args:
            ...
            use_noisy: If False, use nn.Linear instead of NoisyLinear.
        """
        super().__init__()
        c: int
        h: int
        ...
        self.use_noisy: bool = bool(use_noisy)

        # Match _QNet conv stack (channels + activations).
        self.conv: nn.Sequential = nn.Sequential(
            nn.Conv2d(in_channels=int(c), out_channels=16, kernel_size=8, stride=4),
        ...
        self.trunk: nn.Sequential = nn.Sequential(
            nn.Linear(in_features=int(flat_dim), out_features=256),
        ...
        if self.use_noisy:
            self.value: nn.Sequential = nn.Sequential(
                ...
                nn.ReLU(inplace=False),
                NoisyLinear(
                    in_features=64, out_features=self.n_atoms, sigma0=noisy_sigma0
                ),
            )
            ...
            self.value = nn.Sequential(
                ...
                nn.ReLU(inplace=False),
                nn.Linear(in_features=64, out_features=self.n_atoms),
            )
            self.adv = nn.Sequential(
                nn.Linear(in_features=64, out_features=64),
                nn.ReLU(inplace=False),
                nn.Linear(in_features=64, out_features=self.n_actions * self.n_atoms),
            )

    def reset_noise(self) -> None:
        """Reset noise in all NoisyLinear layers (no-op if use_noisy=False)."""
...
class _UniformReplay:
    """Uniform replay buffer with the same sampling interface as prioritized replay."""
    ...
    def sample(
        self,
        batch_size: int,
        beta: float,  # kept for signature compatibility (unused)
        rng: random.Random,
    ) -> tuple[dict[str, np.ndarray], np.ndarray, np.ndarray]:
        """Sample a uniform batch.

        Args:
            batch_size: Batch size.
            beta: Unused (signature compatibility).
            rng: RNG.

        Returns:
            batch: Dict with arrays.
            indices: Indices.
            weights: All-ones weights.
        """
        n: int = int(self.size)
        if n <= 0:
            raise RuntimeError("Cannot sample from an empty buffer.")
        ...
        idx: np.ndarray = np.array(
            [rng.randrange(n) for _ in range(int(batch_size))],
            dtype=np.int64,
        )

        obs_b: np.ndarray = np.stack([self.obs[int(i)] for i in idx], axis=0)  # type: ignore[arg-type]
        next_obs_b: np.ndarray = np.stack([self.next_obs[int(i)] for i in idx], axis=0)  # type: ignore[arg-type]
        ...
        w: np.ndarray = np.ones((int(batch_size),), dtype=np.float32)
        return batch, idx, w
...
class RDQNAgent:
    ...
    def __init__(
        ...
        if bool(cfg.use_per):
            ...
            self.replay = _UniformReplay(capacity=int(cfg.buffer_size))
    ...
    def act(self, obs: np.ndarray, eval_mode: bool) -> int:
        ...
        """
        self.q.eval()
        if (not eval_mode) and bool(self.cfg.use_noisy):
            self.q.reset_noise()
